...
namespace UGame.User.Repository
{
	...
	public class S_channelMO : MySqlTableMO<S_channelEO>
	{
	    ...
	    private void RepairAddData(S_channelEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			...
			sql_ += $" INTO {TableName} (`ChannelID`, `OperatorID`, `CName`, `CType`, `BranchKey`, `FBPixelId`, `FBAccessToken`, `KwaiConfig`, `GAKey`, `TrackConfigs`, `IsStats`, `Note`, `Status`, `RecDate`) VALUE (@ChannelID, @OperatorID, @CName, @CType, @BranchKey, @FBPixelId, @FBAccessToken, @KwaiConfig, @GAKey, @TrackConfigs, @IsStats, @Note, @Status, @RecDate);";
		...
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`ChannelID`, `OperatorID`, `CName`, `CType`, `BranchKey`, `FBPixelId`, `FBAccessToken`, `KwaiConfig`, `GAKey`, `TrackConfigs`, `IsStats`, `Note`, `Status`, `RecDate`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.ChannelID}','{item.OperatorID}','{item.CName}',{item.CType},'{item.BranchKey}','{item.FBPixelId}','{item.FBAccessToken}','{item.KwaiConfig}','{item.GAKey}','{item.TrackConfigs}',{item.IsStats},'{item.Note}',{item.Status},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}'),");
				if (count == batchCount)
				{
					count = 0;
		...
		{
			sql_ = $"UPDATE {TableName} SET `ChannelID` = @ChannelID, `OperatorID` = @OperatorID, `CName` = @CName, `CType` = @CType, `BranchKey` = @BranchKey, `FBPixelId` = @FBPixelId, `FBAccessToken` = @FBAccessToken, `KwaiConfig` = @KwaiConfig, `GAKey` = @GAKey, `TrackConfigs` = @TrackConfigs, `IsStats` = @IsStats, `Note` = @Note, `Status` = @Status WHERE `ChannelID` = @ChannelID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChannelID", item.ChannelID, MySqlDbType.VarChar),
				Database.CreateInParameter("@OperatorID", item.OperatorID != null ? item.OperatorID : (object)DBNull.Value, MySqlDbType.VarChar),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutIsStatsByPK(string channelID, bool isStats, TransactionManager tm_ = null)
		{
			RepairPutIsStatsByPKData(channelID, isStats, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutIsStatsByPKAsync(string channelID, bool isStats, TransactionManager tm_ = null)
		{
			RepairPutIsStatsByPKData(channelID, isStats, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutIsStatsByPKData(string channelID, bool isStats, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `IsStats` = @IsStats  WHERE `ChannelID` = @ChannelID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public bool GetIsStatsByPK(string channelID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChannelID", channelID, MySqlDbType.VarChar),
			};
			return (bool)GetScalar("`IsStats`", "`ChannelID` = @ChannelID", paras_, tm_);
		}
		public async Task<bool> GetIsStatsByPKAsync(string channelID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChannelID", channelID, MySqlDbType.VarChar),
			};
			return (bool)await GetScalarAsync("`IsStats`", "`ChannelID` = @ChannelID", paras_, tm_);
		}
		...
		/// /// <param name = "isStats">是否统计</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats)
		{
			return GetByIsStats(isStats, 0, string.Empty, null);
		}
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats, TransactionManager tm_)
		{
			return GetByIsStats(isStats, 0, string.Empty, tm_);
		}
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats, int top_)
		{
			return GetByIsStats(isStats, top_, string.Empty, null);
		}
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats, int top_, TransactionManager tm_)
		{
			return GetByIsStats(isStats, top_, string.Empty, tm_);
		}
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats, string sort_)
		{
			return GetByIsStats(isStats, 0, sort_, null);
		}
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats, string sort_, TransactionManager tm_)
		{
			return GetByIsStats(isStats, 0, sort_, tm_);
		}
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_channelEO> GetByIsStats(bool isStats, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<S_channelEO>> GetByIsStatsAsync(bool isStats, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`IsStats` = @IsStats", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@IsStats", isStats, MySqlDbType.Byte));
