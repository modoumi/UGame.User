...
namespace UGame.User.Repository
{
	...
	public class S_currency_changeEO : IRowMapper<S_currency_changeEO>
	{
		...
		public static S_currency_changeEO MapDataReader(IDataReader reader)
		{
			...
			ret.AmountCoin = reader.ToInt64("AmountCoin");
			ret.EndCoin = reader.ToInt64("EndCoin");
	...
	public class S_currency_changeMO : MySqlTableMO<S_currency_changeEO>
	{
	    ...
	    private void RepairAddData(S_currency_changeEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			...
			sql_ += $" INTO {TableName} (`ChangeID`, `ProviderID`, `AppID`, `OperatorID`, `UserID`, `FromMode`, `FromId`, `DomainID`, `UserKind`, `CountryID`, `CurrencyID`, `CurrencyType`, `IsBonus`, `FlowMultip`, `Reason`, `PlanAmount`, `Meta`, `SourceType`, `SourceTable`, `SourceId`, `UserIp`, `Status`, `RecDate`, `DealTime`, `DealStatus`, `Amount`, `EndBalance`, `AmountBonus`, `EndBonus`, `AmountCoin`, `EndCoin`, `SettlStatus`, `SettlTable`, `SettlId`, `SettlAmount`) VALUE (@ChangeID, @ProviderID, @AppID, @OperatorID, @UserID, @FromMode, @FromId, @DomainID, @UserKind, @CountryID, @CurrencyID, @CurrencyType, @IsBonus, @FlowMultip, @Reason, @PlanAmount, @Meta, @SourceType, @SourceTable, @SourceId, @UserIp, @Status, @RecDate, @DealTime, @DealStatus, @Amount, @EndBalance, @AmountBonus, @EndBonus, @AmountCoin, @EndCoin, @SettlStatus, @SettlTable, @SettlId, @SettlAmount);";
			paras_ = new List<MySqlParameter>() 
			{
				...
				Database.CreateInParameter("@AmountCoin", item.AmountCoin, MySqlDbType.Int64),
				Database.CreateInParameter("@EndCoin", item.EndCoin, MySqlDbType.Int64),
		...
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`ChangeID`, `ProviderID`, `AppID`, `OperatorID`, `UserID`, `FromMode`, `FromId`, `DomainID`, `UserKind`, `CountryID`, `CurrencyID`, `CurrencyType`, `IsBonus`, `FlowMultip`, `Reason`, `PlanAmount`, `Meta`, `SourceType`, `SourceTable`, `SourceId`, `UserIp`, `Status`, `RecDate`, `DealTime`, `DealStatus`, `Amount`, `EndBalance`, `AmountBonus`, `EndBonus`, `AmountCoin`, `EndCoin`, `SettlStatus`, `SettlTable`, `SettlId`, `SettlAmount`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.ChangeID}','{item.ProviderID}','{item.AppID}','{item.OperatorID}','{item.UserID}',{item.FromMode},'{item.FromId}','{item.DomainID}',{item.UserKind},'{item.CountryID}','{item.CurrencyID}',{item.CurrencyType},{item.IsBonus},{item.FlowMultip},'{item.Reason}',{item.PlanAmount},'{item.Meta}',{item.SourceType},'{item.SourceTable}','{item.SourceId}','{item.UserIp}',{item.Status},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}','{item.DealTime?.ToString("yyyy-MM-dd HH:mm:ss")}','{item.DealStatus}',{item.Amount},{item.EndBalance},{item.AmountBonus},{item.EndBonus},{item.AmountCoin},{item.EndCoin},{item.SettlStatus},'{item.SettlTable}','{item.SettlId}',{item.SettlAmount}),");
				if (count == batchCount)
				{
					count = 0;
		...
		#region RemoveByAmountCoin
		/// <summary>
		/// 按字段删除
		/// </summary>
		/// /// <param name = "amountCoin">实际coin变化（正负数）</param>
		/// <param name="tm_">事务管理对象</param>
		public int RemoveByAmountCoin(long amountCoin, TransactionManager tm_ = null)
		{
			RepairRemoveByAmountCoinData(amountCoin, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> RemoveByAmountCoinAsync(long amountCoin, TransactionManager tm_ = null)
		{
			RepairRemoveByAmountCoinData(amountCoin, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairRemoveByAmountCoinData(long amountCoin, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"DELETE FROM {TableName} WHERE `AmountCoin` = @AmountCoin";
			paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64));
		}
		#endregion // RemoveByAmountCoin
		#region RemoveByEndCoin
		/// <summary>
		/// 按字段删除
		/// </summary>
		/// /// <param name = "endCoin">处理后coin余额</param>
		/// <param name="tm_">事务管理对象</param>
		public int RemoveByEndCoin(long endCoin, TransactionManager tm_ = null)
		{
			RepairRemoveByEndCoinData(endCoin, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> RemoveByEndCoinAsync(long endCoin, TransactionManager tm_ = null)
		{
			RepairRemoveByEndCoinData(endCoin, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairRemoveByEndCoinData(long endCoin, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"DELETE FROM {TableName} WHERE `EndCoin` = @EndCoin";
			paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64));
		}
		#endregion // RemoveByEndCoin
		...
		{
			sql_ = $"UPDATE {TableName} SET `ChangeID` = @ChangeID, `ProviderID` = @ProviderID, `AppID` = @AppID, `OperatorID` = @OperatorID, `UserID` = @UserID, `FromMode` = @FromMode, `FromId` = @FromId, `DomainID` = @DomainID, `UserKind` = @UserKind, `CountryID` = @CountryID, `CurrencyID` = @CurrencyID, `CurrencyType` = @CurrencyType, `IsBonus` = @IsBonus, `FlowMultip` = @FlowMultip, `Reason` = @Reason, `PlanAmount` = @PlanAmount, `Meta` = @Meta, `SourceType` = @SourceType, `SourceTable` = @SourceTable, `SourceId` = @SourceId, `UserIp` = @UserIp, `Status` = @Status, `DealTime` = @DealTime, `DealStatus` = @DealStatus, `Amount` = @Amount, `EndBalance` = @EndBalance, `AmountBonus` = @AmountBonus, `EndBonus` = @EndBonus, `AmountCoin` = @AmountCoin, `EndCoin` = @EndCoin, `SettlStatus` = @SettlStatus, `SettlTable` = @SettlTable, `SettlId` = @SettlId, `SettlAmount` = @SettlAmount WHERE `ChangeID` = @ChangeID_Original";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", item.ChangeID, MySqlDbType.VarChar),
				Database.CreateInParameter("@ProviderID", item.ProviderID != null ? item.ProviderID : (object)DBNull.Value, MySqlDbType.VarChar),
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutDomainIDByPK(string changeID, string domainID, TransactionManager tm_ = null)
		{
			RepairPutDomainIDByPKData(changeID, domainID, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutDomainIDByPKAsync(string changeID, string domainID, TransactionManager tm_ = null)
		{
			RepairPutDomainIDByPKData(changeID, domainID, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutDomainIDByPKData(string changeID, string domainID, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `DomainID` = @DomainID  WHERE `ChangeID` = @ChangeID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutAmountCoinByPK(string changeID, long amountCoin, TransactionManager tm_ = null)
		{
			RepairPutAmountCoinByPKData(changeID, amountCoin, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAmountCoinByPKAsync(string changeID, long amountCoin, TransactionManager tm_ = null)
		{
			RepairPutAmountCoinByPKData(changeID, amountCoin, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutAmountCoinByPKData(string changeID, long amountCoin, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `AmountCoin` = @AmountCoin  WHERE `ChangeID` = @ChangeID";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64),
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutAmountCoin(long amountCoin, TransactionManager tm_ = null)
		{
			string sql_ = $"UPDATE {TableName} SET `AmountCoin` = @AmountCoin";
			var parameter_ = Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64);
			return Database.ExecSqlNonQuery(sql_, new MySqlParameter[] { parameter_ }, tm_);
		}
		public async Task<int> PutAmountCoinAsync(long amountCoin, TransactionManager tm_ = null)
		{
			string sql_ = $"UPDATE {TableName} SET `AmountCoin` = @AmountCoin";
			var parameter_ = Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64);
			return await Database.ExecSqlNonQueryAsync(sql_, new MySqlParameter[] { parameter_ }, tm_);
		}
		#endregion // PutAmountCoin
		#region PutEndCoin
		/// <summary>
		/// 按主键更新列数据
		/// </summary>
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutEndCoinByPK(string changeID, long endCoin, TransactionManager tm_ = null)
		{
			RepairPutEndCoinByPKData(changeID, endCoin, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutEndCoinByPKAsync(string changeID, long endCoin, TransactionManager tm_ = null)
		{
			RepairPutEndCoinByPKData(changeID, endCoin, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutEndCoinByPKData(string changeID, long endCoin, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `EndCoin` = @EndCoin  WHERE `ChangeID` = @ChangeID";
			paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64),
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutEndCoin(long endCoin, TransactionManager tm_ = null)
		{
			string sql_ = $"UPDATE {TableName} SET `EndCoin` = @EndCoin";
			var parameter_ = Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64);
			return Database.ExecSqlNonQuery(sql_, new MySqlParameter[] { parameter_ }, tm_);
		}
		public async Task<int> PutEndCoinAsync(long endCoin, TransactionManager tm_ = null)
		{
			string sql_ = $"UPDATE {TableName} SET `EndCoin` = @EndCoin";
			var parameter_ = Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64);
			return await Database.ExecSqlNonQueryAsync(sql_, new MySqlParameter[] { parameter_ }, tm_);
		}
		#endregion // PutEndCoin
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public string GetDomainIDByPK(string changeID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
			return (string)GetScalar("`DomainID`", "`ChangeID` = @ChangeID", paras_, tm_);
		}
		public async Task<string> GetDomainIDByPKAsync(string changeID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
			return (string)await GetScalarAsync("`DomainID`", "`ChangeID` = @ChangeID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public long GetAmountCoinByPK(string changeID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
			return (long)GetScalar("`AmountCoin`", "`ChangeID` = @ChangeID", paras_, tm_);
		}
		public async Task<long> GetAmountCoinByPKAsync(string changeID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
			return (long)await GetScalarAsync("`AmountCoin`", "`ChangeID` = @ChangeID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public long GetEndCoinByPK(string changeID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
			return (long)GetScalar("`EndCoin`", "`ChangeID` = @ChangeID", paras_, tm_);
		}
		public async Task<long> GetEndCoinByPKAsync(string changeID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@ChangeID", changeID, MySqlDbType.VarChar),
			};
			return (long)await GetScalarAsync("`EndCoin`", "`ChangeID` = @ChangeID", paras_, tm_);
		}
		...
		/// /// <param name = "domainID">域名（不带http）</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID)
		{
			return GetByDomainID(domainID, 0, string.Empty, null);
		}
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID, TransactionManager tm_)
		{
			return GetByDomainID(domainID, 0, string.Empty, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID, int top_)
		{
			return GetByDomainID(domainID, top_, string.Empty, null);
		}
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID, int top_, TransactionManager tm_)
		{
			return GetByDomainID(domainID, top_, string.Empty, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID, string sort_)
		{
			return GetByDomainID(domainID, 0, sort_, null);
		}
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID, string sort_, TransactionManager tm_)
		{
			return GetByDomainID(domainID, 0, sort_, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByDomainID(string domainID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<S_currency_changeEO>> GetByDomainIDAsync(string domainID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(domainID != null ? "`DomainID` = @DomainID" : "`DomainID` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (domainID != null)
		...
		/// /// <param name = "amountCoin">实际coin变化（正负数）</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin)
		{
			return GetByAmountCoin(amountCoin, 0, string.Empty, null);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin)
		{
			return await GetByAmountCoinAsync(amountCoin, 0, string.Empty, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin, TransactionManager tm_)
		{
			return GetByAmountCoin(amountCoin, 0, string.Empty, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin, TransactionManager tm_)
		{
			return await GetByAmountCoinAsync(amountCoin, 0, string.Empty, tm_);
		}
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin, int top_)
		{
			return GetByAmountCoin(amountCoin, top_, string.Empty, null);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin, int top_)
		{
			return await GetByAmountCoinAsync(amountCoin, top_, string.Empty, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin, int top_, TransactionManager tm_)
		{
			return GetByAmountCoin(amountCoin, top_, string.Empty, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin, int top_, TransactionManager tm_)
		{
			return await GetByAmountCoinAsync(amountCoin, top_, string.Empty, tm_);
		}
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin, string sort_)
		{
			return GetByAmountCoin(amountCoin, 0, sort_, null);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin, string sort_)
		{
			return await GetByAmountCoinAsync(amountCoin, 0, sort_, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin, string sort_, TransactionManager tm_)
		{
			return GetByAmountCoin(amountCoin, 0, sort_, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin, string sort_, TransactionManager tm_)
		{
			return await GetByAmountCoinAsync(amountCoin, 0, sort_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByAmountCoin(long amountCoin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`AmountCoin` = @AmountCoin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64));
			return Database.ExecSqlList(sql_, paras_, tm_, S_currency_changeEO.MapDataReader);
		}
		public async Task<List<S_currency_changeEO>> GetByAmountCoinAsync(long amountCoin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`AmountCoin` = @AmountCoin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64));
		...
		/// /// <param name = "endCoin">处理后coin余额</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin)
		{
			return GetByEndCoin(endCoin, 0, string.Empty, null);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin)
		{
			return await GetByEndCoinAsync(endCoin, 0, string.Empty, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin, TransactionManager tm_)
		{
			return GetByEndCoin(endCoin, 0, string.Empty, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin, TransactionManager tm_)
		{
			return await GetByEndCoinAsync(endCoin, 0, string.Empty, tm_);
		}
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin, int top_)
		{
			return GetByEndCoin(endCoin, top_, string.Empty, null);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin, int top_)
		{
			return await GetByEndCoinAsync(endCoin, top_, string.Empty, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin, int top_, TransactionManager tm_)
		{
			return GetByEndCoin(endCoin, top_, string.Empty, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin, int top_, TransactionManager tm_)
		{
			return await GetByEndCoinAsync(endCoin, top_, string.Empty, tm_);
		}
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin, string sort_)
		{
			return GetByEndCoin(endCoin, 0, sort_, null);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin, string sort_)
		{
			return await GetByEndCoinAsync(endCoin, 0, sort_, null);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin, string sort_, TransactionManager tm_)
		{
			return GetByEndCoin(endCoin, 0, sort_, tm_);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin, string sort_, TransactionManager tm_)
		{
			return await GetByEndCoinAsync(endCoin, 0, sort_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_currency_changeEO> GetByEndCoin(long endCoin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`EndCoin` = @EndCoin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64));
			return Database.ExecSqlList(sql_, paras_, tm_, S_currency_changeEO.MapDataReader);
		}
		public async Task<List<S_currency_changeEO>> GetByEndCoinAsync(long endCoin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`EndCoin` = @EndCoin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64));
