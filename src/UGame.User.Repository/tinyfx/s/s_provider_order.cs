...
namespace UGame.User.Repository
{
	...
	public class S_provider_orderMO : MySqlTableMO<S_provider_orderEO>
	{
	    ...
	    private void RepairAddData(S_provider_orderEO item, bool useIgnore_, out string sql_, out List<MySqlParameter> paras_)
		{
			...
			sql_ += $" INTO {TableName} (`OrderID`, `ProviderID`, `AppID`, `OperatorID`, `UserID`, `FromMode`, `FromId`, `DomainID`, `UserKind`, `CountryID`, `CurrencyID`, `CurrencyType`, `PromoterType`, `ReqMark`, `RequestUUID`, `ProviderOrderId`, `ReferProviderOrderId`, `RoundClosed`, `RoundId`, `RewardUUID`, `IsFree`, `PlanBet`, `PlanWin`, `PlanAmount`, `Meta`, `UserIp`, `Status`, `RecDate`, `ResponseTime`, `ResponseStatus`, `Amount`, `EndBalance`, `BetBonus`, `WinBonus`, `EndBonus`, `AmountBonus`, `AmountCoin`, `EndCoin`, `SettlTable`, `SettlId`, `SettlAmount`, `SettlStatus`) VALUE (@OrderID, @ProviderID, @AppID, @OperatorID, @UserID, @FromMode, @FromId, @DomainID, @UserKind, @CountryID, @CurrencyID, @CurrencyType, @PromoterType, @ReqMark, @RequestUUID, @ProviderOrderId, @ReferProviderOrderId, @RoundClosed, @RoundId, @RewardUUID, @IsFree, @PlanBet, @PlanWin, @PlanAmount, @Meta, @UserIp, @Status, @RecDate, @ResponseTime, @ResponseStatus, @Amount, @EndBalance, @BetBonus, @WinBonus, @EndBonus, @AmountBonus, @AmountCoin, @EndCoin, @SettlTable, @SettlId, @SettlAmount, @SettlStatus);";
		...
		{
			var count = 0;
	        var insertSql = $"INSERT INTO {TableName} (`OrderID`, `ProviderID`, `AppID`, `OperatorID`, `UserID`, `FromMode`, `FromId`, `DomainID`, `UserKind`, `CountryID`, `CurrencyID`, `CurrencyType`, `PromoterType`, `ReqMark`, `RequestUUID`, `ProviderOrderId`, `ReferProviderOrderId`, `RoundClosed`, `RoundId`, `RewardUUID`, `IsFree`, `PlanBet`, `PlanWin`, `PlanAmount`, `Meta`, `UserIp`, `Status`, `RecDate`, `ResponseTime`, `ResponseStatus`, `Amount`, `EndBalance`, `BetBonus`, `WinBonus`, `EndBonus`, `AmountBonus`, `AmountCoin`, `EndCoin`, `SettlTable`, `SettlId`, `SettlAmount`, `SettlStatus`) VALUES ";
			var sql = new StringBuilder();
	        foreach (var item in items)
			{
				count++;
				sql.Append($"('{item.OrderID}','{item.ProviderID}','{item.AppID}','{item.OperatorID}','{item.UserID}',{item.FromMode},'{item.FromId}','{item.DomainID}',{item.UserKind},'{item.CountryID}','{item.CurrencyID}',{item.CurrencyType},{item.PromoterType},{item.ReqMark},'{item.RequestUUID}','{item.ProviderOrderId}','{item.ReferProviderOrderId}',{item.RoundClosed},'{item.RoundId}','{item.RewardUUID}',{item.IsFree},{item.PlanBet},{item.PlanWin},{item.PlanAmount},'{item.Meta}','{item.UserIp}',{item.Status},'{item.RecDate.ToString("yyyy-MM-dd HH:mm:ss")}','{item.ResponseTime?.ToString("yyyy-MM-dd HH:mm:ss")}','{item.ResponseStatus}',{item.Amount},{item.EndBalance},{item.BetBonus},{item.WinBonus},{item.EndBonus},{item.AmountBonus},{item.AmountCoin},{item.EndCoin},'{item.SettlTable}','{item.SettlId}',{item.SettlAmount},{item.SettlStatus}),");
				if (count == batchCount)
				{
					count = 0;
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutAmountCoinByPK(string orderID, long amountCoin, TransactionManager tm_ = null)
		{
			RepairPutAmountCoinByPKData(orderID, amountCoin, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutAmountCoinByPKAsync(string orderID, long amountCoin, TransactionManager tm_ = null)
		{
			RepairPutAmountCoinByPKData(orderID, amountCoin, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutAmountCoinByPKData(string orderID, long amountCoin, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `AmountCoin` = @AmountCoin  WHERE `OrderID` = @OrderID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>受影响的行数</return>
		public int PutEndCoinByPK(string orderID, long endCoin, TransactionManager tm_ = null)
		{
			RepairPutEndCoinByPKData(orderID, endCoin, out string sql_, out List<MySqlParameter> paras_);
			return Database.ExecSqlNonQuery(sql_, paras_, tm_);
		}
		public async Task<int> PutEndCoinByPKAsync(string orderID, long endCoin, TransactionManager tm_ = null)
		{
			RepairPutEndCoinByPKData(orderID, endCoin, out string sql_, out List<MySqlParameter> paras_);
			return await Database.ExecSqlNonQueryAsync(sql_, paras_, tm_);
		}
		private void RepairPutEndCoinByPKData(string orderID, long endCoin, out string sql_, out List<MySqlParameter> paras_)
		{
			sql_ = $"UPDATE {TableName} SET `EndCoin` = @EndCoin  WHERE `OrderID` = @OrderID";
			paras_ = new List<MySqlParameter>() 
			{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public long GetAmountCoinByPK(string orderID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OrderID", orderID, MySqlDbType.VarChar),
			};
			return (long)GetScalar("`AmountCoin`", "`OrderID` = @OrderID", paras_, tm_);
		}
		public async Task<long> GetAmountCoinByPKAsync(string orderID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OrderID", orderID, MySqlDbType.VarChar),
			};
			return (long)await GetScalarAsync("`AmountCoin`", "`OrderID` = @OrderID", paras_, tm_);
		}
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return></return>
		public long GetEndCoinByPK(string orderID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OrderID", orderID, MySqlDbType.VarChar),
			};
			return (long)GetScalar("`EndCoin`", "`OrderID` = @OrderID", paras_, tm_);
		}
		public async Task<long> GetEndCoinByPKAsync(string orderID, TransactionManager tm_ = null)
		{
			var paras_ = new List<MySqlParameter>() 
			{
				Database.CreateInParameter("@OrderID", orderID, MySqlDbType.VarChar),
			};
			return (long)await GetScalarAsync("`EndCoin`", "`OrderID` = @OrderID", paras_, tm_);
		}
		...
		/// /// <param name = "domainID">域名（不带http）</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID)
		{
			return GetByDomainID(domainID, 0, string.Empty, null);
		}
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID, TransactionManager tm_)
		{
			return GetByDomainID(domainID, 0, string.Empty, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID, int top_)
		{
			return GetByDomainID(domainID, top_, string.Empty, null);
		}
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID, int top_, TransactionManager tm_)
		{
			return GetByDomainID(domainID, top_, string.Empty, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID, string sort_)
		{
			return GetByDomainID(domainID, 0, sort_, null);
		}
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID, string sort_, TransactionManager tm_)
		{
			return GetByDomainID(domainID, 0, sort_, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByDomainID(string domainID, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<S_provider_orderEO>> GetByDomainIDAsync(string domainID, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL(domainID != null ? "`DomainID` = @DomainID" : "`DomainID` IS NULL", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			if (domainID != null)
		...
		/// /// <param name = "amountCoin">实际coin变化（正负数）</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin)
		{
			return GetByAmountCoin(amountCoin, 0, string.Empty, null);
		}
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin, TransactionManager tm_)
		{
			return GetByAmountCoin(amountCoin, 0, string.Empty, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin, int top_)
		{
			return GetByAmountCoin(amountCoin, top_, string.Empty, null);
		}
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin, int top_, TransactionManager tm_)
		{
			return GetByAmountCoin(amountCoin, top_, string.Empty, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin, string sort_)
		{
			return GetByAmountCoin(amountCoin, 0, sort_, null);
		}
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin, string sort_, TransactionManager tm_)
		{
			return GetByAmountCoin(amountCoin, 0, sort_, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByAmountCoin(long amountCoin, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<S_provider_orderEO>> GetByAmountCoinAsync(long amountCoin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`AmountCoin` = @AmountCoin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@AmountCoin", amountCoin, MySqlDbType.Int64));
		...
		/// /// <param name = "endCoin">处理后coin余额</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin)
		{
			return GetByEndCoin(endCoin, 0, string.Empty, null);
		}
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin, TransactionManager tm_)
		{
			return GetByEndCoin(endCoin, 0, string.Empty, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin, TransactionManager tm_)
		{
		...
		/// <param name = "top_">获取行数</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin, int top_)
		{
			return GetByEndCoin(endCoin, top_, string.Empty, null);
		}
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin, int top_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin, int top_, TransactionManager tm_)
		{
			return GetByEndCoin(endCoin, top_, string.Empty, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin, int top_, TransactionManager tm_)
		{
		...
		/// <param name = "sort_">排序表达式</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin, string sort_)
		{
			return GetByEndCoin(endCoin, 0, sort_, null);
		}
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin, string sort_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin, string sort_, TransactionManager tm_)
		{
			return GetByEndCoin(endCoin, 0, sort_, tm_);
		}
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin, string sort_, TransactionManager tm_)
		{
		...
		/// <param name="tm_">事务管理对象</param>
		/// <return>实体对象集合</return>
		public List<S_provider_orderEO> GetByEndCoin(long endCoin, int top_, string sort_, TransactionManager tm_)
		{
		...
		public async Task<List<S_provider_orderEO>> GetByEndCoinAsync(long endCoin, int top_, string sort_, TransactionManager tm_)
		{
			var sql_ = BuildSelectSQL("`EndCoin` = @EndCoin", top_, sort_);
			var paras_ = new List<MySqlParameter>();
			paras_.Add(Database.CreateInParameter("@EndCoin", endCoin, MySqlDbType.Int64));
